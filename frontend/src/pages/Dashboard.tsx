  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
        ...
        <div className="bg-slate-900 p-10 rounded-[2.5rem] shadow-2xl shadow-slate-300 text-white flex flex-col justify-between relative overflow-hidden h-full">
          <div className="absolute bottom-0 right-0 w-64 h-64 bg-indigo-600 opacity-20 blur-[100px] rounded-full translate-x-1/2 translate-y-1/2"></div>
        ...
        <div className="h-full">
          {eligibleRequests.length > 0 && profile.isAvailable ? (
            <div className="space-y-4 h-full flex flex-col">
              ...
              <div className="flex-grow overflow-y-auto pr-2 space-y-4 max-h-[400px] scrollbar-hide">
                {eligibleRequests.map((req: any) => (
                  <div key={req.id} className="bg-white p-6 rounded-[2rem] shadow-xl shadow-red-100/50 border border-red-50 border-l-4 border-l-red-500 flex flex-col justify-between group hover:shadow-2xl transition-all duration-300">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="text-lg font-black text-slate-900 leading-tight uppercase">{req.hospitalName}</h3>
                        <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-wider">{req.treatmentType}</p>
                      </div>
                      <div className="bg-red-50 px-3 py-1 rounded-xl border border-red-100">
                        <span className="text-red-600 font-black text-xs">{req.bloodGroup}</span>
                      </div>
                    </div>
                    <div className="flex items-center justify-between mt-2">
                      <div className="text-sm text-slate-500 font-medium">
                        Patient: <span className="font-bold text-slate-800">{req.patientName || 'Anonymous'}</span>
                      </div>
                      <div className="text-sm text-slate-500 font-medium">
                        Units: <span className="font-bold text-slate-800">{req.quantity}</span>
                      </div>
                    </div>
                    <button
                      onClick={() => handleAcceptRequest(req)}
                      disabled={acceptingId === req.id}
                      className={`mt-6 w-full py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all shadow-lg active:scale-95 ${acceptingId === req.id ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-red-500 hover:bg-red-600 text-white shadow-red-200'}`}
                    >
            ...
            <div className="bg-white p-10 rounded-[2.5rem] shadow-xl shadow-slate-100 border border-slate-100 h-full flex flex-col items-center justify-center text-center">
              ...
              <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">No active {studentDetails.bloodGroup} requests</p>
      ...
      <div className="flex items-center justify-center py-4 bg-white/50 rounded-2xl border border-slate-100">
        <p className="text-sm font-black text-slate-400 uppercase tracking-[0.3em]">
          Current Points: <span className="text-3xl text-slate-900 ml-2">{profile.trustScore}</span>
        </p>
      </div>
